...
| Render-blocking requests (~610 ms, LCP/FCP) | Enabled `experimental.inlineCss` (Next 16) — route CSS inlined into HTML; verified 0 `<link rel=stylesheet>` in built pages | `next.config.mjs` |
| Improve image delivery (~24 KiB hero, LCP) | Added AVIF: `images.formats: ['image/avif','image/webp']` (sharp present for prod) | `next.config.mjs` |
| Reduce unused JS + long tasks | **Not actioned — "Unscored" diagnostics.** Biggest item is Google's own `gtag.js` (66 KiB), unshrinkable; already deferred via `lazyOnload` (E1). Low ROI. | — |

**Indexing robustness fix (July 10):** `getBlogPost` (`src/lib/api/blog.ts`) previously returned `null` on *any* backend error, so a 5xx/timeout during a crawl or ISR revalidation would emit `noindex` + `notFound()` on a **real** published post. Now it returns `null` only on a genuine **404**, and **throws on 5xx/network/timeout** so Next serves the last-good ISR page instead of baking a bad `noindex`. (Discovered via a Bing "NOINDEX" report that turned out to be a stale pre-slug-rename crawl of `/blog/free-food-apis`, not a live issue.) A full seeded-vs-live slug audit of all 82 posts found exactly **one** rename — `free-food-apis-2025` → `free-food-apis` (done in the CMS, not a migration) — now 301-redirected in `next.config.mjs`. No other renames, no orphaned/admin-created slugs, no internal links to the dead URL.
